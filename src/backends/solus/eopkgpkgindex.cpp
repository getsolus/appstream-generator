...
std::string EopkgPackageIndex::downloadIfNecessary(const std::string &fname, const std::string &tempDir)
{
    ...
    try {
        ...
        LOG_ERROR(m_log, "Unable to download: {}", e.what());
        throw;
    }
    ...
    const std::string &arch)
{
    ...
    if (!doc) {
        LOG_ERROR(m_log, "Failed to parse repository index XML");
        return {};
    }
    ...
    if (!pisiNode || std::strcmp(reinterpret_cast<const char *>(pisiNode->name), "PISI") != 0) {
        LOG_ERROR(m_log, "Repository index does not contain a PISI root element");
        xmlFreeDoc(doc);
        return packages;
    }
    ...
    for (xmlNodePtr packageNode = pisiNode->children; packageNode; packageNode = packageNode->next) {
        ...
        if (currentPkgName.empty()) {
            LOG_WARNING(m_log, "Skipping package entry without a name.");
        ...
        if (currentPkgName.ends_with("-devel") || currentPkgName.ends_with("-dbginfo")) {
            LOG_DEBUG(m_log, "Skipping development/debug package: {}", currentPkgName);
            continue;
        }
        ...
        for (xmlNodePtr child = packageNode->children; child; child = child->next) {
            ...
            if (std::strcmp(childName, "History") == 0) {
                ...
                if (!packageURI.empty()) {
                    ...
                    LOG_DEBUG(m_log, "Package path: {}", pkgPath);
                }
        ...
        if (pkg->isValid()) {
            ...
            LOG_ERROR(
                m_log,
                "Found an invalid package entry for '{}' (name, architecture or version is missing). Skipping it.",
                pkg->name());
        }
    }
    ...
    const std::string &section)
{
    ...
    try {
        ...
        LOG_ERROR(m_log, "Failed to process package file '{}': {}", fname, e.what());
        return nullptr;
    }
}
    ...
    const std::string &arch)
{
    ...
    try {
        ...
        LOG_DEBUG(m_log, "Failed to parse stored mtime: {}", e.what());
