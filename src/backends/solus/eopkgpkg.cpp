...
std::string EopkgPackage::downloadIfNecessary()
{
    ...
    if (Utils::isRemote(m_pkgFname)) {
        ...
        try {
            ...
            LOG_ERROR(logBackend, "Unable to download: {}, reason {}", m_pkgFname, e.what());
            throw;
        }
...
void EopkgPackage::extractMetadata()
{
    ...
    try {
        ...
        LOG_ERROR(logBackend, "Failed to parse eopkg metadata: {}", e.what());
        throw;
    }
}
...
std::vector<std::uint8_t> EopkgPackage::getFileData(const std::string &fname)
{
    ...
    try {
        ...
        LOG_WARNING(logBackend, "File '{}' not found in install.tar.xz: {}", actualPath, e.what());
        return {};
    }
}
...
void EopkgPackage::cleanupTempDir(const fs::path &path)
{
    try {
        ...
        LOG_DEBUG(logBackend, "Unable to remove temporary directory: {} ({})", path.string(), e.what());
    }
}
...
void EopkgPackage::finish()
{
    ...
    try {
        if (Utils::isRemote(m_pkgFname) && !m_localPkgFname.empty() && fs::exists(m_localPkgFname)) {
            LOG_DEBUG(logBackend, "Deleting temporary package file {}", m_localPkgFname);
            fs::remove(m_localPkgFname);
            m_localPkgFname.clear();
        }
    } catch (const std::exception &e) {
        // we ignore any error
        LOG_DEBUG(logBackend, "Unable to remove temporary package: {} ({})", m_localPkgFname, e.what());
    }
}
